...
import { computePart, bufferColor, isMilestone, gaicStatus, type PartMeta } from './calc.ts';
...
const names: Record<string, string> = {
  '7P21': '検査（素材確認）', '8209': 'サーメテルWコ－ティング',
  '7P31': '検査（浸透探傷）', '8A61': '不働態化処理', '7P32': '検査（処理）',
};
const resolve = (shop: string) => names[shop] ?? `Shop ${shop}`;

const R = (i: number, shop: string, wip = false, extra: Partial<RoutingRow> = {}): RoutingRow => ({
  osId: 'X000677148', seqMain: i, seqSub: 0, seqLabel: String(i), shop, job: '0000',
  planStart: d('2026-01-01'), planEnd: d('2026-06-01'), wip,
  materialStatus: '4_材料払出済', outDate: null, inDate: null, etaDate: null, orderNo: '', ...extra,
});
...
const meta: PartMeta = {
  osId: 'X000677148', partNo: '37B93029-003', name: 'FITTING', category: '機構部品', kishu: '37B',
  finalDue: d('2026-07-10'), urgent: false, shortage: false,
};
