...
import type { Color, GaicStatus, Part, RoutingRow, TimelineCell } from './types.ts';
import type { MilestoneRule } from './masters.ts';
...
interface Cell {
  shop: string; job: string; planStart: Date | null; planEnd: Date | null;
  wip: boolean; gaic: boolean; materialStatus: string;
  hasIn: boolean; hasOut: boolean; hasEta: boolean; orderNo: string;
}
...
function compress(rows: RoutingRow[]): Cell[] {
  ...
  for (const r of rows) {
    ...
    if (last && last.shop === r.shop) {
      ...
      if (r.outDate) {
        last.gaic = true; last.hasOut = true; last.materialStatus = r.materialStatus;
        last.hasIn = last.hasIn || !!r.inDate; last.hasEta = last.hasEta || !!r.etaDate;
        if (r.orderNo) last.orderNo = r.orderNo;
      }
      continue;
    }
    cells.push({
      shop: r.shop, job: r.job, planStart: r.planStart, planEnd: r.planEnd, wip: r.wip,
      gaic: !!r.outDate, materialStatus: r.materialStatus,
      hasIn: !!r.inDate, hasOut: !!r.outDate, hasEta: !!r.etaDate, orderNo: r.orderNo,
    });
  }
  return cells;
}
...
export interface PartMeta {
  osId: string; partNo: string; name: string; category: string; kishu: string;
  finalDue: Date | null; urgent: boolean; shortage: boolean;
}

export function computePart(
  meta: PartMeta, rows: RoutingRow[], resolveName: (shop: string, job: string) => string,
  asOf: Date, o: CalcOptions,
): Part {
  ...
  const stagnant = currentCell && currentCell.planStart
    ? Math.max(0, dayDiff(asOf, currentCell.planStart, o.holidays)) : 0;
    ...
    if (isMilestone(c.shop, name, o.milestoneRules)) {
      ...
      if (!passed) {
        const msDue = finalDue
          ? new Date(finalDue.getTime() - (total - 1 - i) * o.milestoneLtDays * 86400000) : null;
  ...
  return {
    id: meta.osId, partNo: meta.partNo, name: meta.name, category: meta.category, kishu: meta.kishu,
    finalDue: ymd(finalDue), daysLeft, totalShops: total, doneShops, remainShops,
    buffer, color, stagnant, urgent: meta.urgent, shortage: meta.shortage,
    currentShop, timeline, inst: meta.osId.replace(/\D/g, '').slice(-4),
  };
}
